/*-----------------------------------------------------------------------------
 ...
 Descript. : This program calculates the matrix sum given two others matrices
             with random numbers through threads using PTHREADS
-----------------------------------------------------------------------------*/
...
//thread function called to sum two input matrices
void *matrix_sum(void *tid)
{
    int thid;           /*thread ID*/
    int i,j;            /*iterators*/
...
int main(void)
{
    ...
    int i,j,th, tmili;              /*thread iterators and time of execution*/
    ...
    {
        pthread_create(&t[th], NULL, &matrix_sum, (void *) th);
    }
